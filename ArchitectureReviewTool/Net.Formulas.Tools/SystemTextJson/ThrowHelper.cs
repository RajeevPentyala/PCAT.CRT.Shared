...
namespace System.Text.Json
{
    internal static partial class ThrowHelper
    {
        ...
        {
            return GetInvalidOperationException(_buffered);// SR.Format(SR.CallFlushToAvoidDataLoss, _buffered));
        }
        ...
        public static void ThrowInvalidOperationException(int currentDepth)
        {
            ...
            ThrowInvalidOperationException("DepthTooLarge");// SR.Format(SR.DepthTooLarge, currentDepth, JsonConstants.MaxWriterDepth));
        }
        ...
        private static InvalidOperationException GetInvalidOperationException(int currentDepth)
        {
            ...
            if (currentDepth != 0)
            {
                return GetInvalidOperationException("ZeroDepthAtEnd");// SR.Format(SR.ZeroDepthAtEnd, currentDepth));
            }
            else
            {
                return GetInvalidOperationException("EmptyJsonIsInvalid");// SR.EmptyJsonIsInvalid);
            }
        }
        ...
        public static void ThrowInvalidOperationOrArgumentException(ReadOnlySpan<char> propertyName, int currentDepth)
        {
            ...
            {
                Debug.Assert(propertyName.Length > JsonConstants.MaxCharacterTokenSize);
                ThrowArgumentException("PropertyNameTooLarge");// SR.Format(SR.PropertyNameTooLarge, propertyName.Length));
            }
        }
        ...
        {
            return GetInvalidOperationException("CannotSkip");// SR.CannotSkip);
        }

        [MethodImpl(MethodImplOptions.NoInlining)]
        private static InvalidOperationException GetInvalidOperationException(string message, JsonTokenType tokenType)
        {
            return GetInvalidOperationException("InvalidCast");// SR.Format(SR.InvalidCast, tokenType, message));
        }

        [MethodImpl(MethodImplOptions.NoInlining)]
        private static InvalidOperationException GetInvalidOperationException(JsonTokenType tokenType)
        {
            return GetInvalidOperationException("InvalidComparison");// SR.Format(SR.InvalidComparison, tokenType));
        }
        ...
        {
            return GetInvalidOperationException("JsonElementHasWrongType");// SR.Format(SR.JsonElementHasWrongType, expectedType, actualType));
        }
        ...
        internal static InvalidOperationException GetJsonElementWrongTypeException(
            ...
            JsonValueKind actualType)
        {
            return GetInvalidOperationException("JsonElementHasWrongType expectedTypeName");// SR.Format(SR.JsonElementHasWrongType, expectedTypeName, actualType));
        }
        ...
        private static string GetResourceString(ref Utf8JsonReader json, ExceptionResource resource, byte nextByte, string characters)
        {
            ...
            switch (resource)
            {
                case ExceptionResource.ArrayDepthTooLarge:
                    message = "ArrayDepthTooLarge";// SR.Format(SR.ArrayDepthTooLarge, json.CurrentState.Options.MaxDepth);
                    break;
                case ExceptionResource.MismatchedObjectArray:
                    message = "MismatchedObjectArray";//SR.Format(SR.MismatchedObjectArray, character);
                    break;
                case ExceptionResource.TrailingCommaNotAllowedBeforeArrayEnd:
                    message = "TrailingCommaNotAllowedBeforeArrayEnd";//SR.TrailingCommaNotAllowedBeforeArrayEnd;
                    break;
                case ExceptionResource.TrailingCommaNotAllowedBeforeObjectEnd:
                    message = "TrailingCommaNotAllowedBeforeObjectEnd";//SR.TrailingCommaNotAllowedBeforeObjectEnd;
                    break;
                case ExceptionResource.EndOfStringNotFound:
                    message = "EndOfStringNotFound";//SR.EndOfStringNotFound;
                    break;
                case ExceptionResource.RequiredDigitNotFoundAfterSign:
                    message = "RequiredDigitNotFoundAfterSign";//SR.Format(SR.RequiredDigitNotFoundAfterSign, character);
                    break;
                case ExceptionResource.RequiredDigitNotFoundAfterDecimal:
                    message = "RequiredDigitNotFoundAfterDecimal";//SR.Format(SR.RequiredDigitNotFoundAfterDecimal, character);
                    break;
                case ExceptionResource.RequiredDigitNotFoundEndOfData:
                    message = "RequiredDigitNotFoundEndOfData";//SR.RequiredDigitNotFoundEndOfData;
                    break;
                case ExceptionResource.ExpectedEndAfterSingleJson:
                    message = "ExpectedEndAfterSingleJson";//SR.Format(SR.ExpectedEndAfterSingleJson, character);
                    break;
                case ExceptionResource.ExpectedEndOfDigitNotFound:
                    message = "ExpectedEndOfDigitNotFound";//SR.Format(SR.ExpectedEndOfDigitNotFound, character);
                    break;
                case ExceptionResource.ExpectedNextDigitEValueNotFound:
                    message = "ExpectedNextDigitEValueNotFound";//SR.Format(SR.ExpectedNextDigitEValueNotFound, character);
                    break;
                case ExceptionResource.ExpectedSeparatorAfterPropertyNameNotFound:
                    message = "ExpectedSeparatorAfterPropertyNameNotFound";//SR.Format(SR.ExpectedSeparatorAfterPropertyNameNotFound, character);
                    break;
                case ExceptionResource.ExpectedStartOfPropertyNotFound:
                    message = "ArrayDepthTooLarge";//SR.Format(SR.ExpectedStartOfPropertyNotFound, character);
                    break;
                case ExceptionResource.ExpectedStartOfPropertyOrValueNotFound:
                    message = "ExpectedStartOfPropertyOrValueNotFound";//SR.ExpectedStartOfPropertyOrValueNotFound;
                    break;
                case ExceptionResource.ExpectedStartOfPropertyOrValueAfterComment:
                    message = "ExpectedStartOfPropertyOrValueAfterComment";//SR.Format(SR.ExpectedStartOfPropertyOrValueAfterComment, character);
                    break;
                case ExceptionResource.ExpectedStartOfValueNotFound:
                    message = "ExpectedStartOfValueNotFound";//SR.Format(SR.ExpectedStartOfValueNotFound, character);
                    break;
                case ExceptionResource.ExpectedValueAfterPropertyNameNotFound:
                    message = "ExpectedValueAfterPropertyNameNotFound";//SR.ExpectedValueAfterPropertyNameNotFound;
                    break;
                case ExceptionResource.FoundInvalidCharacter:
                    message = "FoundInvalidCharacter";//SR.Format(SR.FoundInvalidCharacter, character);
                    break;
                case ExceptionResource.InvalidEndOfJsonNonPrimitive:
                    message = "InvalidEndOfJsonNonPrimitive";//SR.Format(SR.InvalidEndOfJsonNonPrimitive, json.TokenType);
                    break;
                case ExceptionResource.ObjectDepthTooLarge:
                    message = "ObjectDepthTooLarge";//SR.Format(SR.ObjectDepthTooLarge, json.CurrentState.Options.MaxDepth);
                    break;
                case ExceptionResource.ExpectedFalse:
                    message = "ExpectedFalse";//SR.Format(SR.ExpectedFalse, characters);
                    break;
                case ExceptionResource.ExpectedNull:
                    message = "ExpectedNull";//SR.Format(SR.ExpectedNull, characters);
                    break;
                case ExceptionResource.ExpectedTrue:
                    message = "ExpectedTrue";//SR.Format(SR.ExpectedTrue, characters);
                    break;
                case ExceptionResource.InvalidCharacterWithinString:
                    message = "InvalidCharacterWithinString";//SR.Format(SR.InvalidCharacterWithinString, character);
                    break;
                case ExceptionResource.InvalidCharacterAfterEscapeWithinString:
                    message = "InvalidCharacterAfterEscapeWithinString";//SR.Format(SR.InvalidCharacterAfterEscapeWithinString, character);
                    break;
                case ExceptionResource.InvalidHexCharacterWithinString:
                    message = "InvalidHexCharacterWithinString";//SR.Format(SR.InvalidHexCharacterWithinString, character);
                    break;
                case ExceptionResource.EndOfCommentNotFound:
                    message = "EndOfCommentNotFound";//SR.EndOfCommentNotFound;
                    break;
                case ExceptionResource.ZeroDepthAtEnd:
                    message = "ZeroDepthAtEnd";//SR.Format(SR.ZeroDepthAtEnd);
                    break;
                case ExceptionResource.ExpectedJsonTokens:
                    message = "ExpectedJsonTokens";//SR.ExpectedJsonTokens;
                    break;
                case ExceptionResource.NotEnoughData:
                    message = "NotEnoughData";//SR.NotEnoughData;
                    break;
                case ExceptionResource.ExpectedOneCompleteToken:
                    message = "ExpectedOneCompleteToken";//SR.ExpectedOneCompleteToken;
                    break;
                case ExceptionResource.InvalidCharacterAtStartOfComment:
                    message = "InvalidCharacterAtStartOfComment";//SR.Format(SR.InvalidCharacterAtStartOfComment, character);
                    break;
                case ExceptionResource.UnexpectedEndOfDataWhileReadingComment:
                    message = "UnexpectedEndOfDataWhileReadingComment";// SR.Format(SR.UnexpectedEndOfDataWhileReadingComment);
                    break;
                case ExceptionResource.UnexpectedEndOfLineSeparator:
                    message = "UnexpectedEndOfLineSeparator";//SR.Format(SR.UnexpectedEndOfLineSeparator);
                    break;
                case ExceptionResource.InvalidLeadingZeroInNumber:
                    message = "InvalidLeadingZeroInNumber";//SR.Format(SR.InvalidLeadingZeroInNumber, character);
                    break;
                default:
        ...
        {
            throw GetInvalidOperationException(charAsInt);//SR.Format(SR.CannotReadInvalidUTF16, $"0x{charAsInt:X2}"));
        }
            ...
            switch (resource)
            {
                case ExceptionResource.MismatchedObjectArray:
                    ...
                    message = (tokenType == JsonTokenType.PropertyName) ?
                        "CannotWriteEndAfterProperty"://SR.Format(SR.CannotWriteEndAfterProperty, (char)token) :
                        "MismatchedObjectArray";//SR.Format(SR.MismatchedObjectArray, (char)token);
                    break;
                case ExceptionResource.DepthTooLarge:
                    message = "DepthTooLarge";// SR.Format(SR.DepthTooLarge, currentDepth & JsonConstants.RemoveFlagsBitMask, JsonConstants.MaxWriterDepth);
                    break;
                case ExceptionResource.CannotStartObjectArrayWithoutProperty:
                    message = "CannotStartObjectArrayWithoutProperty";// SR.Format(SR.CannotStartObjectArrayWithoutProperty, tokenType);
                    break;
                case ExceptionResource.CannotStartObjectArrayAfterPrimitiveOrClose:
                    message = "CannotStartObjectArrayAfterPrimitiveOrClose";// SR.Format(SR.CannotStartObjectArrayAfterPrimitiveOrClose, tokenType);
                    break;
                case ExceptionResource.CannotWriteValueWithinObject:
                    message = "CannotWriteValueWithinObject";// SR.Format(SR.CannotWriteValueWithinObject, tokenType);
                    break;
                case ExceptionResource.CannotWritePropertyWithinArray:
                    message = (tokenType == JsonTokenType.PropertyName) ?
                        "CannotWritePropertyAfterProperty" ://SR.Format(SR.CannotWritePropertyAfterProperty) :
                        "CannotWritePropertyWithinArray";//SR.Format(SR.CannotWritePropertyWithinArray, tokenType);
                    break;
                case ExceptionResource.CannotWriteValueAfterPrimitiveOrClose:
                    message = "CannotWriteValueAfterPrimitiveOrClose"; //SR.Format(SR.CannotWriteValueAfterPrimitiveOrClose, tokenType);
                    break;
                default:
                    Debug.Fail($"The ExceptionResource enum value: {resource} is not part of the switch. Add the appropriate case and exception message.");
                    break;
        ...
        public static FormatException GetFormatException(DataType dateType)
        {
            ...
            switch (dateType)
            {
                case DataType.DateTime:
                    message = "SR.FormatDateTime";//SR.FormatDateTime;
                    break;
                case DataType.DateTimeOffset:
                    message = "SR.FormatDateTimeOffset";//SR.FormatDateTimeOffset;
                    break;
                case DataType.Base64String:
                    message = "SR.CannotDecodeInvalidBase64";//SR.CannotDecodeInvalidBase64;
                    break;
                case DataType.Guid:
                    message = "SR.FormatGuid";//SR.FormatGuid;
                    break;
                default:
