...
namespace System.Text.Json
{
    internal static partial class JsonHelpers
    {
        ...
        private static bool TryParseDateTimeOffset(ReadOnlySpan<byte> source, out DateTimeParseData parseData)
        {
            ...
            bool ParseOffset(ref DateTimeParseData parseDatae, ReadOnlySpan<byte> offsetData)
            {
                // Parse the hours for the offset
                if (offsetData.Length < 2
                    || !TryGetNextTwoDigits(offsetData.Slice(0, 2), ref parseDatae.OffsetHours))
                {
                    return false;
                }
                ...
                if (offsetData.Length != 5
                    ...
                    || !TryGetNextTwoDigits(offsetData.Slice(3), ref parseDatae.OffsetMinutes))
                {
                    return false;
                }
