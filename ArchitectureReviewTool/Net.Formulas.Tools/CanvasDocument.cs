...
namespace Microsoft.PowerPlatform.Formulas.Tools
{
    ...
    public class CanvasDocument
    {
        ...
        internal string FullAppcheckecontent;
        ...
        /// <param name="fullPathToMsApp">path to an .msapp file</param>
        /// <returns>A tuple of the document and errors and warnings. If there are errors, the document is null.  </returns>
        public static (CanvasDocument, ErrorContainer) LoadFromMsappStream(MemoryStream msappFile,ITracingService tracingService)
        {
            var errors = new ErrorContainer();
            var doc = Wrapper(() => MsAppSerializer.Load(msappFile, errors/*, tracingService*/), errors);
            return (doc, errors);
        }


        public static (CanvasDocument, ErrorContainer) LoadFromMsappStream(MemoryStream msappFile)
        {
            var errors = new ErrorContainer();
            var doc = Wrapper(() => MsAppSerializer.Load(msappFile, errors), errors);
            return (doc, errors);
        }
